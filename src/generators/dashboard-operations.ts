...
function backfillProjectionConfidence(state, workspaceId) {
  if (isPlainObject(state.projectionConfidence)) return;
  const meta = state.meta || {};
  const evidence = state.governanceEvidenceBrief || {};
  const trust = state.trustBoundary || {};
  const missingEvidenceClaims = Array.isArray(evidence.missingEvidenceClaims) ? evidence.missingEvidenceClaims : [];
  const unresolvedDecisions = Array.isArray(evidence.unresolvedDecisions) ? evidence.unresolvedDecisions : [];
  state.projectionConfidence = {
    ...
    localListenerStatus: String((state.listener || {}).status || "not-started"),
    actionGate: "Run dashboard-ops refresh before treating this legacy projection as operational truth.",
    userMessage: "This dashboard state predates projection-confidence fields and was backfilled for safe validation.",
    requiredActions: [
      "Run dashboard-ops verify-projections",
      "Run dashboard-ops refresh",
      "Record a governed session with request/process/result trace"
    ],
    evidenceRefs: ["legacy-dashboard-state", workspaceId]
  };
}

function backfillSessionTraceability(state, workspaceId, workspaceName, purpose) {
  if (isPlainObject(state.sessionTraceability) && Array.isArray(state.sessionTraceability.entries) && state.sessionTraceability.entries.length > 0) return;
  ...
  const entries = sourceSessions.length > 0
    ? sourceSessions.slice(0, 10).map((session) => {
        ...
        return {
          ...
          evidenceRefs,
          nextStep: String(session.nextStep || "Run dashboard-ops refresh and record the next governed session."),
...
function validateState(state) {
  ...
  const projectionConfidence = requireValidationObject(errors, "dashboardState.projectionConfidence", state.projectionConfidence) || {};
  requireValidationString(errors, "dashboardState.projectionConfidence.status", projectionConfidence.status);
  requireValidationString(errors, "dashboardState.projectionConfidence.completeness", projectionConfidence.completeness);
  requireValidationString(errors, "dashboardState.projectionConfidence.staleness", projectionConfidence.staleness);
  requireValidationString(errors, "dashboardState.projectionConfidence.trustBoundaryStatus", projectionConfidence.trustBoundaryStatus);
  requireValidationNumber(errors, "dashboardState.projectionConfidence.missingEvidenceCount", projectionConfidence.missingEvidenceCount);
  requireValidationNumber(errors, "dashboardState.projectionConfidence.openDecisionCount", projectionConfidence.openDecisionCount);
  requireValidationString(errors, "dashboardState.projectionConfidence.actionGate", projectionConfidence.actionGate);
  requireValidationStringArray(errors, "dashboardState.projectionConfidence.requiredActions", projectionConfidence.requiredActions);
  const sessionTraceability = requireValidationObject(errors, "dashboardState.sessionTraceability", state.sessionTraceability) || {};
  requireValidationString(errors, "dashboardState.sessionTraceability.status", sessionTraceability.status);
  requireValidationString(errors, "dashboardState.sessionTraceability.purpose", sessionTraceability.purpose);
  const traceEntries = Array.isArray(sessionTraceability.entries) ? sessionTraceability.entries : [];
  if (!Array.isArray(sessionTraceability.entries) || traceEntries.length === 0) {
